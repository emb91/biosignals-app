-- Drop the unused fiber_* columns now that Fiber AI is removed (2026-06-09).
-- The data in them is a duplicate of the apollo_* payloads (0 rows ever had
-- source='fiber'), so this is data-safe. The columns are entangled with the
-- contacts→people compat layer, so the order matters:
--   1. drop the dependent reporting view (data_provider_usage_by_user)
--   2. drop + recreate the `contacts` compat view without the 5 fiber columns
--      (CREATE OR REPLACE VIEW can't drop columns), restoring owner + grants
--   3. recreate its 3 INSTEAD OF triggers
--   4. recreate the 4 functions that referenced fiber, minus fiber
--   5. drop the columns from the base tables (people, contacts_legacy)
--   6. recreate the reporting view
-- Reproduced from the LIVE definitions as of 2026-06-09; if the contacts-split
-- migrations (20260604/05) are later changed, reconcile this file with them.

-- 1. dependent reporting view (added 2026-06-09) — must go before the contacts view
drop view if exists public.data_provider_usage_by_user;

-- 2. recreate the compat view without fiber_* columns
...
-- 3. recreate the INSTEAD OF triggers (dropped with the view)
...
-- 4a. compat insert — fiber columns removed from the people INSERT
...
-- 4b. compat update — fiber assignments removed from the people UPDATE
...
-- 4c. apply_person_enrichment — fiber assignments removed
...
-- 4d. import_upsert_contact — fiber columns removed from INSERT + ON CONFLICT
...
BEGIN
  IF v_lnk IS NULL OR btrim(v_lnk) = '' THEN
    RETURN;  -- no canonical key: caller treats row as not-imported
  END IF;
...
-- 5. drop the columns from the base tables
alter table public.people
  drop column fiber_person_response_raw,
  drop column fiber_company_response_raw,
  drop column fiber_person_raw,
  drop column fiber_company_raw,
  drop column fiber_lookup_metadata;

alter table public.contacts_legacy
  drop column fiber_person_response_raw,
  drop column fiber_company_response_raw,
  drop column fiber_person_raw,
  drop column fiber_company_raw,
...
-- 6. recreate the reporting view (unchanged — it never referenced fiber)
