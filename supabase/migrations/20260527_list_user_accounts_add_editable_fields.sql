-- Add editable companies fields that the original list_user_accounts RPC was
-- dropping (website, industry, sub_industry, clinical_stage, headquarters_state,
-- platform_category, company_size_bucket, tagline). Without these in the RPC,
-- the edit dialog and side panel both received `undefined`, so the form showed
-- "— Unset" placeholders and the side panel had no UI for the fields.
--
-- Function return type changes → must DROP then CREATE (CREATE OR REPLACE
-- refuses to change return types).
...
RETURNS TABLE(
  id uuid, company_name text, domain text, website text, logo_url text,
