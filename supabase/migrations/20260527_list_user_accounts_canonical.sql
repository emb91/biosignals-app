-- Phase 1e — fix the accounts RPCs after companies.user_id and the per-user
-- columns were dropped in 20260527_companies_canonical_drop_user_id.sql.
--
-- list_user_accounts and get_account_page_for_company used to read
-- company_fit_score, company_fit_coverage, and matched_icp_id from `companies`.
-- Those fields now live on `user_companies` (uc); the joins already exist, so
-- this is purely a column-source change.
...
RETURNS TABLE(
  id uuid, company_name text, domain text, logo_url text,
  company_fit_score double precision, company_fit_coverage double precision,
  matched_icp_id uuid,
  ...
  headquarters_city text, headquarters_country text, founded_year integer,
  ...
  avg_contact_fit double precision, max_contact_intent_score double precision,
...
LANGUAGE sql STABLE AS $function$
  WITH contact_agg AS (
    SELECT
      ...
      MAX(CASE WHEN intent_score > 0 THEN intent_score ELSE NULL END) AS max_contact_intent_score
    FROM contacts
  ...
  SELECT
    f.id, f.company_name, f.domain, f.logo_url,
...
CREATE OR REPLACE FUNCTION public.get_account_page_for_company(
  p_user_id uuid, p_company_id uuid, p_page_size integer DEFAULT 50
)
    ...
    WHERE uc.user_id     = p_user_id
      ...
      AND (
        ...
        OR (
          ...
          AND uc.company_fit_score > COALESCE(target.company_fit_score, -1)
        )
      )
  )
