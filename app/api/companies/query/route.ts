...
const SYSTEM_PROMPT = `${TASK_AGENT_OPENING} Interpret natural language queries about companies and return structured JSON.
...
Computed company coverage status:
- "opportunity": company_fit_score >= 0.6 and best_contact_fit is missing or < 1.0; a good company where they need a 100% contact match
...
- Add "source" only if the user asks where the company data came from.
...
- "covered companies" or "covered accounts" -> coverageStatuses: ["covered"]
  ...
  try {
    ...
    if (sorted.length === 0 && !claudeResult.conversational) {
      return NextResponse.json({
        ...
        accounts: [],
        conversational:
          "I couldn't find any companies matching that. Try broadening your search or adjusting the filters.",
      } satisfies AgentAccountsQueryResult);
    }
