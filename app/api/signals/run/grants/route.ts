...
type SyncSummary = {
  ran: boolean;
  ok: boolean;
  result: SyncNihGrantsDeltaResult | null;
  error: string | null;
};

const GRANTS_ONLY_KEYS = new Set<SignalKey>(['grant_award']);
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as RunGrantsBody;
    ...
    if (body.sync_first === true) {
      ...
      try {
        ...
        syncSummary.result = await syncNihGrantsDelta({ admin, overlapDays: syncOverlapDays });
        syncSummary.ok = true;
      } catch (error) {
        syncSummary.ok = false;
        syncSummary.error = messageFromUnknown(error);
        console.error('[signals/run/grants] sync_first failed (continuing with stale data):', error);
      }
    }

    let result: Awaited<ReturnType<typeof runGrantsMonitor>>;
    ...
    if (runAll && requestedCompanyIds.length === 0) {
      ...
      for (let i = 0; i < allIds.length; i += batchSize) {
        ...
        try {
          const chunkResult = await runGrantsMonitor({
            userId: user.id,
            companyIds: chunk,
            onlySignalKey: onlySignalKey && GRANTS_ONLY_KEYS.has(onlySignalKey) ? onlySignalKey : undefined,
          });
    ...
    await persistRunHistory(createAdminClient(), {
      userId: user.id,
      signalKey: onlySignalKey && GRANTS_ONLY_KEYS.has(onlySignalKey) ? onlySignalKey : 'grants_all',
      runner: 'grants',
      scope: 'company',
    ...
    console.error('[signals/run/grants] error:', error);
