...
import { syncPressReleases, type SyncPressReleasesResult } from '@/lib/signals/sync-press-releases';
...
type SyncSummary = {
  ran: boolean;
  ok: boolean;
  result: SyncPressReleasesResult | null;
  error: string | null;
};
...
export async function POST(request: Request) {
  try {
    ...
    const syncLookbackDays =
      typeof body.sync_lookback_days === 'number' && Number.isFinite(body.sync_lookback_days)
        ? Math.max(1, Math.trunc(body.sync_lookback_days))
        : 7;
    ...
    if (body.sync_first === true) {
      ...
      try {
        ...
        syncSummary.result = await syncPressReleases({ admin, lookbackDays: syncLookbackDays });
        syncSummary.ok = true;
      } catch (error) {
        syncSummary.ok = false;
        syncSummary.error = messageFromUnknown(error);
        console.error('[signals/run/press-releases] sync_first failed:', error);
      }
    }

    let result: Awaited<ReturnType<typeof runPressReleaseMonitor>>;
    ...
    if (runAll && requestedCompanyIds.length === 0) {
      ...
      for (let i = 0; i < allIds.length; i += batchSize) {
        ...
        try {
          const chunkResult = await runPressReleaseMonitor({
            userId: user.id,
            companyIds: chunk,
            onlySignalKey,
          });
      ...
      result = await runPressReleaseMonitor({
        userId: user.id,
    ...
    await persistRunHistory(createAdminClient(), {
      userId: user.id,
      signalKey: onlySignalKey ?? 'press_releases_all',
