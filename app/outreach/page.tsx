        ...
        {(editorOpen || detailsOpen) && (
          <>
            ...
            {editorOpen && editDraft && editingSequenceId && (
              ...
              >
                {(() => {
                  const isInvite = isLinkedInInvite(editDraft);
                  return (
                    <>
                      <div className="flex items-center justify-between border-b border-[rgba(13,53,71,0.08)] px-5 py-3">
                        <div>
                          ...
                          <h3 className="text-base font-semibold text-[#0d3547]">
                            {isInvite ? 'LinkedIn invite' : 'Edit message'}
                          </h3>
                        </div>
                        <button
                          type="button"
                      ...
                      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                        {isInvite ? (
                          /* Day 7 LinkedIn invite is a pure action — lemlist sends the connect
                             request without a personalised note in our template, so there's
                             nothing to edit here. Just explain what happens. */
                          <div className="rounded-xl border border-[#0a66c2]/20 bg-[#0a66c2]/5 p-4 space-y-3">
                            <div className="flex items-center gap-2">
                              <Linkedin className="h-4 w-4 text-[#0a66c2]" />
                              <p className="text-[13px] font-semibold text-[#0a66c2]">
                                Send LinkedIn invite
                              </p>
                            </div>
                            <p className="text-[12.5px] leading-snug text-[#4a6470]">
                              On Day 7, lemlist sends a LinkedIn connect request to this contact.
                              There&apos;s no copy to write here, the request goes out as a plain connect
                              with no personalised note.
                            </p>
                            <p className="text-[11.5px] leading-snug text-[#7d909a]">
                              If the invite is accepted, lemlist sends the Day 8 LinkedIn message next.
                              If it&apos;s ignored, lemlist still fires the rest of the email cadence on
                              schedule.
                            </p>
                          </div>
                        ) : (
