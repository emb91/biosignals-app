    ...
    <div className="mb-6 shrink-0 flex flex-col gap-4 max-[767px]:pl-14 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
          {total > 0
            ? `${total.toLocaleString()} signal${total !== 1 ? 's' : ''} detected. Click a row for the evidence, readiness context, and next action.`
            : 'Externally monitored readiness signals will appear here as Arcova detects meaningful change.'}
        </p>
      </div>
            ...
            {loadingSignals ? (
              ...
              <>
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-16 text-center shadow-sm">
                  ...
                  <h3 className="mb-2 text-lg font-semibold text-gray-900">No signals yet</h3>
                  <p className="mx-auto mb-6 max-w-sm text-gray-500">
                    Run the signal monitor or wait for the next scheduled pass. External readiness changes will land here.
                  </p>
                  <button
                      ...
                      <div className="border-b border-[rgba(13,53,71,0.08)] bg-[rgba(255,255,255,0.32)] px-4 py-3">
                        <div className="flex items-center justify-between gap-3">
                          <p className="text-sm font-medium text-slate-700">
                            Use Arcova Agent to search names, companies, or specific signal types.
                          </p>
                          ...
                          return (
                            ...
                            >
                              <div className="min-w-0">
                                ...
                                <div className="mt-1 truncate text-[12px] text-slate-500">{secondaryLine(row)}</div>
                              </div>
                              ...
                              <div className="min-w-0">
                                {row.companyId && row.companyName ? (
                                  <button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      router.push(withQuery(ROUTES.leads.accounts, `companyId=${encodeURIComponent(row.companyId!)}`));
                                    }}
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                  {selectedSignal.companyId ? (
                                    <button
                                      type="button"
                                      onClick={() => router.push(withQuery(ROUTES.leads.accounts, `companyId=${encodeURIComponent(selectedSignal.companyId!)}`))}
                                  ...
                                  {selectedSignal.contactId ? (
                                    <button
                                      type="button"
                                      onClick={() => router.push(withQuery(ROUTES.leads.contacts, `lead=${encodeURIComponent(selectedSignal.contactId!)}`))}
                                      className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-[#1f2937] transition-colors hover:bg-gray-50"
                                    >
