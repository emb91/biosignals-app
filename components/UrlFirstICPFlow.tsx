...
export default function UrlFirstICPFlow({ onComplete }: Props) {
  ...
  const toggle = (field: keyof Pick<ReviewState, 'therapeuticAreas' | 'modalities' | 'developmentStages' | 'companySizes' | 'fundingStages'>) =>
    (value: string) =>
      setReview((prev) => ({
        ...prev,
  ...
  const handleAnalyse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const detectedName = data.companyName || normalized.replace(/^https?:\/\/(www\.)?/, '').replace(/\/.*$/, '');
  ...
  const handleSave = async () => {
    ...
    const { name } = nameRes.ok ? await nameRes.json() : { name: `${review.companyType} Profile` };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pt-2">
        <button
          type="button"
          onClick={() => { setUrl(''); setPhase('url-input'); }}
