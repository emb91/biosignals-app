...
### BUILT 2026-06-25 — #1 bulk people match + #2 headcount signal

**#1 Bulk people match (verified live).** `bulkMatchPeopleWithApollo` + `bulkEnrichContactsWithApollo` in `lib/apollo.ts` (people/bulk_match, 10/call); shared `mapApolloEnrichment` so bulk and single produce identical results; per-row personal-email reveal stays a follow-up for the no-email minority (bulk reveal is async-webhook). `enrichContacts()` in `lib/enrichment-provider.ts`. Wired into `lib/import-queue.ts` `processQueuedRowsInBackground` as a **self-healing prefetch**: bulk-match the whole batch up front, take only rows that matched a real person, fall back to per-row `people/match` for the rest and on any bulk error (so behaviour is identical to the single path, at most one wasted bulk call). Benefits all three enrich entry points (CSV, HubSpot pull, triage-room enrich) since they funnel through this loop. Verified: confirmed the triage-room flow (HubSpot first-sync → triage → `awaiting_enrichment` waiting room → user enriches high/medium) and live-tested `people/bulk_match` (2 records, both matched, work email + LinkedIn resolved).

**#2 Headcount-expansion signal (verified live).** New readiness signal `headcount_expansion` (dimension `new_people`, catalog impact 42, decay 180d) sourced from Apollo's `organization_headcount_*_growth` (free on org enrich). `lib/signals/run-headcount-monitor.ts` (`extractHeadcountGrowth` + `runHeadcountSignalForCompany`): emits when 12-month growth ≥ 20%, monthly dedup key `apollo_org_enrich:{companyId}:headcount_expansion:{YYYY-MM}`, mirrors the hiring-monitor emit pattern (insert source event → normalize → recompute). Growth captured in `lib/company-enrichment.ts` (`CompanyEnrichmentResult.headcount_growth`) and emitted per-linked-user from the `company-enrichment-queue` cron. Verified: emit → normalize (correct `new_people` dimension) → dedup (2nd call = duplicate); test signal cleaned up.

**Needs your eyes (tunable, low-blast-radius):** the headcount **threshold (20% / 12mo)** and **impact (42)** are conservative guesses — review against real readiness scoring. Also: it currently emits only from the `company-enrichment-queue` cron (covers imported + re-enriched companies); extending to the full monitored universe would be a periodic headcount cron reading persisted growth (would need growth columns on `companies`). Apollo's growth unit (ratio vs percent) is normalized defensively (`toPercent`).

---

## Contacts CRM tab — "CRM updates" feed (design import, 2026-06-25)

The imported contacts side-panel design adds a **"CRM updates" feed** under the deal in the CRM tab — a timeline of CRM change events (deal created, new contact added, closed-lost, title change, etc.). Deferred from the drawer restyle (the restyle shipped header/tabs/Fit-ring/CRM-copy + the Action two-card layout; this feed is the remaining design delta for the CRM tab).

**Source decision (for now): `signal_source_events`.** It's the only *populated* CRM-event source today — 36 real rows across sources `hubspot_crm_contacts` / `hubspot_crm_deals`: `new_contact_added_in_crm` (22), `open_opportunity_in_crm` (7), `title_change` (3), `closed_lost_in_crm` (2), `new_internal_role` (1), `recently_changed_company` (1). Already humanized (`title`/`summary`) and the same stream `/api/today/crm-activity` renders. Scope the feed to **contact (`entity_contact_id`) OR its company (`entity_company_id`)** — only ~5/36 events are contact-scoped, so a contact-only query is empty for most people; CRM/deal events are company-scoped.

**Build pieces:**
- *Change:* extend `app/api/contacts/[id]/hubspot-crm/route.ts` to also return `updates: [...]` — query `signal_source_events` (the two CRM sources) for `entity_contact_id = contact.id` OR `entity_company_id = contact.company_id`, order by `event_at`/`observed_at` desc, limit ~8, map to `{ id, eventType: source_event_type, title, summary, eventAt }`.
- *New:* render the feed under the deal in the CRM tab as the design's update rows (`.upd`): icon by `source_event_type`, `title`/`summary`, relative time. Keep copy customer-facing (no Apify/Apollo/internal terms).
- *Reuse:* the mapping/normalization pattern in `app/api/today/crm-activity/route.ts`.

**Related gap (flagged, separate) — `crm_deal_stage_history` is empty.** Not a sync failure: deals sync fine (`crm_deals` = 7 rows, daily). The writer `recordDealStageTransition` is wired ([readiness-hubspot-deals.ts:545](lib/signals/readiness-hubspot-deals.ts:545)) but **change-only / going-forward**: the existing deals were mirrored into `crm_deals` before stage capture began, so each sync sees `previous.stage === current.stage` → nothing recorded, and trial data has had no stage moves since. Consequences: (a) a clean stage-transition feed isn't available, and (b) `/coverage` sales-cycle-length (`lib/coverage/icp-performance.ts` reads this table) is starved. Fix = backfill from HubSpot deal property history (`hs_v2_date_entered_*` / property-history API) so past transitions populate; after that, stage transitions become a richer source for this feed. Until then, history only appears for future organic stage changes.
